...
from bids2ebrains.core import convert_bids, scan_missing, patch_openminds, upload_to_kg, validate_jsonld
from bids2ebrains.grouper import group_subjects
from bids2ebrains.mappings import LICENSE, ACCESSIBILITY, AGE_CATEGORY
from bids2ebrains.validator import get_last_validation_mode
    ...
    with st.form("patch_form"):
        ...
        if "DatasetVersion.license" in prompts:
            ...
            lic_choice = st.selectbox(
                ...
                lic_labels, index=0, key="dv_license",
                help="Pick an openMINDS License or paste a full IRI."
            )

            if lic_choice and lic_choice != "(choose)":
        ...
        if "DatasetVersion.accessibility" in prompts:
            ...
            acc_choice = st.selectbox(
                ...
                acc_labels, index=0, key="dv_access",
                help="Pick an openMINDS ProductAccessibility or paste a full IRI."
            )

            if acc_choice and acc_choice != "(choose)":
        ...
        if "SubjectState.ageCategory" in prompts or "SubjectGroupState.ageCategory" in prompts:
            ...
            if age_choice and age_choice != "(choose)":
                if age_choice == "Enter IRI manually":
                    ...
                    answers["SubjectGroupState.ageCategory"] = age_choice

        v = _maybe_text(
            "DatasetVersion.versionIdentifier",
            "DatasetVersion.versionIdentifier",
            "v1.0.0",
            "Short identifier for this dataset version."
        )
        ...
        v = _maybe_text(
            ...
            "DatasetVersion.versionInnovation",
            "Initial release converted from BIDS",
            "Short note describing what is new in this version (e.g., 'Initial release', 'Added 2 subjects', etc.)"
        )
        if v: answers["DatasetVersion.versionInnovation"] = v
        ...
        if submitted:
            if not answers:
                ...
                st.caption(f"Validation mode: {get_last_validation_mode()}")
